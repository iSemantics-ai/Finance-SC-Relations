...
from sc_classifier.config.core import config, EMB_DIR
from sc_classifier.data.dataset import EmbeddingsDict
...
class SimCSE(object):
    ...
    def build_index(self, sentences_or_file_path: Union[str, List[str]], 
        ...
        feature = config.ml_model_config.features[0]
        
        embeddings = EmbeddingsDict(feature, 'embeddings') 
        pre_computed = False 
        ...
        if isinstance(sentences_or_file_path, str):
            if Path(sentences_or_file_path).suffix == '.pt': 
                logger.info("loading pre-computed embeddings")
                embeddings.load(EMB_DIR / sentences_or_file_path )
                assert list(embeddings.keys()) == [feature , 'embeddings']
                pre_computed = True 
                ...
                with open(sentences_or_file_path, "r") as f:
                    ...
                    for line in tqdm(f):
                        embeddings[feature].append(line.rstrip())
        else: 
            embeddings.set_text(sentences_or_file_path)

        if not pre_computed:      
            logger.info("Encoding embeddings for sentences...")
            embeddings['embeddings'] = self.encode(embeddings[feature], device=device, batch_size=batch_size, normalize_to_unit=True, return_numpy=True)
            if save_as:
                embeddings.save(EMB_DIR / save_as)
        
        logger.info(embeddings)
        logger.info("Building index...")
        self.index = {"sentences": embeddings[feature]}
        if use_faiss:
            quantizer = faiss.IndexFlatIP(embeddings['embeddings'].shape[1])  
            if faiss_fast:
                index = faiss.IndexIVFFlat(quantizer, embeddings['embeddings'].shape[1], min(self.num_cells, len(embeddings[feature]))) 
            else:
                index = quantizer
            ...
            if faiss_fast:            
                index.train(embeddings['embeddings'].astype(np.float32))
            index.add(embeddings['embeddings'].astype(np.float32))
            index.nprobe = min(self.num_cells_in_search, len(embeddings[feature]))
    ...
    def augment_dataset(self, 
                        ...
                        top_k:int=10)->Tuple[List[str],List[int], np.ndarray]:
        """Augment samples of dataset 
        
        Args: 

        dataset(pd.DataFrame ,Dataset): the training dataset to be augmented based in model results.
        theshold(float): the minimum similariety score in order to accept any sample. 
        top_k(int): maximum number of similar sentences for each sample. 

        Returns: 
        Tuple[similar_text, labels, original_score]
        """
        similar_texts = self.search(samples[config.ml_model_config.features[0]].tolist(), threshold=sim_threshold, top_k=top_k)
        sim_sents = []
        for aug, label , score in zip(similar_texts, samples[config.ml_model_config.target], samples['Scores']):
            sents = [s[0] for s in aug]
            if len(sents) > 0:
                labels = [label]*len(sents)
                scores = [score]*len(sents)
                sim_sents+= list(zip(sents,labels,scores))

        return pd.DataFrame(sim_sents, columns=samples.columns).drop_duplicates([config.ml_model_config.features[0]]).reset_index(drop=True)
