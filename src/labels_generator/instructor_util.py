...
def relations_tupled_2(relations_list:List[dict]):
    """Converts a list of relations into a list of tuples.
    ...
    for relation_dict in relations_list:
        for relation in relation_dict.get('supplier_and_customer', []):
        ...
        for relation in relation_dict.get('financial_trade', []):
        ...
        for relation in relation_dict.get('nothing', []):
            c1 = relation[0] if len(relation) == 2 else ""
            c2 = relation[1] if len(relation) == 2 else ""
...
def relation_search(query_relation: Tuple[str, str, str],
    ...
    """
    Search for a relation in a given list of relation tuples using a spaCy text matcher.
    ...
    if source_orgs:
        ...
        if source_match:
            dist_match = True if matcher.similarity(query_relation[2],
                                                    ...
                                                    > threshold or\
                                                    any([query_relation[2].lower() in x.lower() for x in source_orgs[source_list[max_idx]]])\
                                                    else False
    ...
    if query_relation[1] in main_relations:
        ...
        if dist_match:
            source_match = True if matcher.similarity(query_relation[2],
                                                    dist_orgs[dist_list[max_idx]]).max()\
                                                    > threshold\
                                                    or any([query_relation[2].lower() in x.lower() for x in dist_orgs[dist_list[max_idx]]])\
                                                    else False
...
def establish_company_relations(datapoint,
                                matcher,
                                feature_key:str="sentence",
                                relations_key:str='relations'):
    
    """Create re-ready-dataset for relation extraction tranining

    ## Example
    datapoint['org_groups'] = {"Apple": 0
                                "DigitHome Inc: 1}
    data[relations_key] = [("Apple", supplier, "DigitHome")] 

    # Now we need to create rows of training data using these examples
    ...
    '''Generate augmentations using similarity scores. 
'''
